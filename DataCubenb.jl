...
# ╔═╡ 2eee01b8-e708-11ec-0332-21cc0ce99f1a
begin
	using Plots
	plotly()
	theme(:gruvbox_dark)
...
# ╔═╡ 9dc40325-ed63-4233-99f3-1a4cb97305fd
begin
...
# ╔═╡ d381e5b4-75c2-4c5d-845a-f978c64ac193
function CrTop(n,m,p=50,q=50,seed=rand(1:10))
	pnt = [[perlinsnoise(x/p,y/q,seed) for x=0:n] for y=0:m]
	top = hcat(pnt...)
	return top
end

# ╔═╡ 1c2b4b58-d7a3-4a5a-9d0a-d33cce4db75c
top = CrTop(200,200)*10; surface(top, zlim=(-100,100))

# ╔═╡ 05f5ee8d-c29d-4c5b-89db-ebef718c01e8
size(top)

# ╔═╡ ee9c78fc-38f8-4bf6-8dfb-4660fe15a6c2
gradx,grady = [diff(top, dims=1)[:,begin:end-1], diff(top, dims=2)[begin:end-1,:]];

# ╔═╡ 7d58b634-8912-41ec-8a84-1e52c1012c6b
begin
	a = []
	for i in eachindex(gradx)
		...
		dx=dx/M
		dy=dy/M
		normal = [dx, dy, 1-dx^2-dy^2]
		push!(a, normal)
	end
	a = reshape(a, (200,200))
end

# ╔═╡ 88c85573-2b8e-405e-8979-bbff409d3cc8
begin
	sun = [0,0,1]
	inten = []
	...
	outiten=(reshape(inten, (200,200)).-minimum(inten))*10
	sun
end

# ╔═╡ abb10580-587a-41da-a615-0238e26d5aae
surface(outiten, zlim=(0,5))

# ╔═╡ a321deed-cacc-4557-ad1f-e15a631127cb
function CrEM(bands::Int64, seed::Int64 = 10)
	coefs = randn(seed)
	freq = randn(seed)
	ar = Vector{Float64}(undef, bands)
	tmpfn(i::Float64) = (map(sin, freq*i)+map(cos, freq*i))'coefs
	@inbounds for i=1:bands
		ar[i] = tmpfn(i/7)
	end
	return (ar/norm(ar)).^2
end

# ╔═╡ 9fbc70ab-30a1-4e02-9bcd-e77ac1eba2af
begin
	EMs = []
	NoEm = 5
	for i=1:NoEm
		push!(EMs, CrEM(200,300))
	end
	plot(EMs; layout=(NoEm÷2,1), legend=false, colorbar=false, showaxis=false, ticks=false, )
end

# ╔═╡ 2dd055cf-d4ae-42ab-8c69-71c7bc548d76
function CrMix(n,m,EMs)
	
	Ar = Array{Float64}(undef, n, m, length(EMs[1]))
	for i in CartesianIndices(Ar[:,:,1])
		Ar[i,:] = sum(rand(length(EMs)) .* EMs)
	end
	
	return Ar
end

# ╔═╡ 2627283f-0d6d-48c0-b900-b33d6938430d
function CrMix2(n,m,EMs)
	
	Ar = zeros(Float64, n, m ,length(EMs[1])) # Array{Float64}(nothing, n, m, length(EMs[1]))
	for EM in EMs
		MixMat = CrTop(n-1,m-1,100,100)
		MixMat .-= minimum(MixMat)
		MixMat /= maximum(MixMat)
		for i in CartesianIndices(MixMat)
			Ar[i,:] += MixMat[i]*EM
		end
	end
	return Ar
...
# ╔═╡ 14b130f6-b23c-42ae-a5fb-f67f13934937
result = CrMix2(200,200,EMs) .* outiten * 10;

# ╔═╡ db3d7193-f087-4664-86b9-2f0bccdc046d
surface(result[:,:,150], zlim=(-100,100))

# ╔═╡ 574ad8f2-056f-46c2-b156-5b8052c158f5
@save "/home/rnarwar/Pixxel/project/testdata/FabricatedDataCube/EMs.jld2" EMs

# ╔═╡ 68de1225-2272-4a28-bdda-5bb24f2b8700
@save "/home/rnarwar/Pixxel/project/testdata/FabricatedDataCube/Image.jld2" result

# ╔═╡ 62303ba8-741f-4b87-b4bc-d7ef8b815baa
struct IPixel
	Ind::CartesianIndex
	Vec::Vector{UInt8}
end

# ╔═╡ c2b00342-f9d8-4bfd-a96f-2d9dc7d55a9d
...
# ╔═╡ c5c96470-e2ae-42c3-a1dd-360779fc1e6b
...
# ╔═╡ 5f0b6689-efed-4615-9b0b-249d0680ec5a
...
# ╔═╡ 77eb9475-989c-4c68-b4f0-50f2c31b99cd
...
# ╔═╡ 02c5012f-45d8-47fa-89b0-4d62f9ae9b92
...
# ╔═╡ b04f1926-ea97-4da6-9cdb-ec9f7602f5b4
...
# ╔═╡ 673ddd3d-5625-4d8f-af06-7b19e0d7a65b
begin
	madeupimage = Flatten(load("/home/rnarwar/Pixxel/project/testdata/FabricatedDataCube/Image.jld2", "result"))
	@save "/home/rnarwar/Pixxel/project/testdata/FabricatedDataCube/FlatImage.jld2" madeupimage
end
